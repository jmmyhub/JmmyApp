package com.jmmy.jmmyapp.ble.blemanager;
...
import com.jmmy.jmmyapp.ble.BLEDevice;
import com.jmmy.jmmyapp.ble.util.ClsUtils;
...
public class BLEManager {
    ...
    private BluetoothAdapter.LeScanCallback leScanCallback = new BluetoothAdapter.LeScanCallback() {
        ...
        public void onLeScan(BluetoothDevice bluetoothDevice, int rssi, byte[] bytes) {
            ...
            if (bluetoothDevice.getName() != null) {
                LogUtil.d(TAG, bluetoothDevice.getName() + "-->" + bluetoothDevice.getAddress());
            } else {
                LogUtil.d(TAG, "null" + "-->" + bluetoothDevice.getAddress());
            }
    ...
    public void startDiscoveryDevice(OnDeviceSearchListener onDeviceSearchListener, long scanTime) {
        if (bluetooth4Adapter == null) {
            LogUtil.e(TAG, "startDiscoveryDevice-->bluetooth4Adapter == null");
            return;
        }
    ...
    public void stopDiscoveryDevice() {
        ...
        if (bluetooth4Adapter == null) {
            LogUtil.e(TAG, "stopDiscoveryDevice-->bluetooth4Adapter == null");
            return;
        }

        if (leScanCallback == null) {
            LogUtil.e(TAG, "stopDiscoveryDevice-->leScanCallback == null");
            return;
        }

        LogUtil.d(TAG, "停止扫描设备");
    ...
    private BluetoothGattCallback bluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            switch (status) {
                case BluetoothGatt.GATT_SUCCESS:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_SUCCESS");
                    break;
                case BluetoothGatt.GATT_FAILURE:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_FAILURE");
                    break;
                case BluetoothGatt.GATT_CONNECTION_CONGESTED:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_CONNECTION_CONGESTED");
                    break;
                case BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_INSUFFICIENT_AUTHENTICATION");
                    break;
                case BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_INSUFFICIENT_ENCRYPTION");
                    break;
                case BluetoothGatt.GATT_INVALID_OFFSET:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_INVALID_OFFSET");
                    break;
                case BluetoothGatt.GATT_READ_NOT_PERMITTED:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_READ_NOT_PERMITTED");
                    break;
                case BluetoothGatt.GATT_REQUEST_NOT_SUPPORTED:
                    LogUtil.w(TAG, "BluetoothGatt.GATT_REQUEST_NOT_SUPPORTED");
                    break;
            }
            ...
            LogUtil.d(TAG, "连接的设备：" + bluetoothDevice.getName() + "  " + bluetoothDevice.getAddress());
            ...
            if (newState == BluetoothGatt.STATE_CONNECTED) {
                ...
                LogUtil.e(TAG, "断开连接status:" + status);
                gatt.close();  //断开连接释放连接
                ...
                if (status == 133) {
                    ...
                    if (onBleConnectListener != null) {
                        ...
                        LogUtil.e(TAG, "连接失败status：" + status + "  " + bluetoothDevice.getAddress());
                    }
                } else if (status == 62) {
                    //成功连接没有发现服务断开
                    if (onBleConnectListener != null) {
                        ...
                        LogUtil.e(TAG, "连接成功服务未发现断开status:" + status);
                    }
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            ...
            if (characteristic.getValue() == null) {
                LogUtil.e(TAG, "characteristic.getValue() == null");
                return;
            }
            //将收到的字节数组转换成十六进制字符串
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            ...
            LogUtil.w("TAG", "收到数据str:" + TypeConversion.bytes2HexString(bytes, bytes.length));
            if (onBleConnectListener != null) {
        ...
        public void onReadRemoteRssi(BluetoothGatt gatt, int rssi, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtil.w(TAG, "读取RSSI值成功，RSSI值：" + rssi + ",status" + status);
                if (onBleConnectListener != null) {
                    onBleConnectListener.onReadRssi(gatt, rssi, status);  //成功读取连接的信号强度回调
                }
            } else if (status == BluetoothGatt.GATT_FAILURE) {
                LogUtil.w(TAG, "读取RSSI值失败，status：" + status);
            }
        }
        ...
        public void onMtuChanged(BluetoothGatt gatt, int mtu, int status) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {  //设置MTU成功
                ...
                //mtu - 3
                LogUtil.w(TAG, "设置MTU成功，新的MTU值：" + (mtu - 3) + ",status" + status);
                if (onBleConnectListener != null) {
                ...
                LogUtil.e(TAG, "设置MTU值失败：" + (mtu - 3) + ",status" + status);
                if (onBleConnectListener != null) {
    ...
    public BluetoothGatt connectBleDevice(Context context, BluetoothDevice bluetoothDevice, long outTime, String serviceUUID, String readUUID, String writeUUID, OnBleConnectListener onBleConnectListener) {
        if (bluetoothDevice == null) {
            LogUtil.e(TAG, "connectBleDevice()-->bluetoothDevice == null");
            return null;
        }
        if (isConnectIng) {
            LogUtil.e(TAG, "connectBleDevice()-->isConnectIng = true");
            return null;
        }
        ...
        LogUtil.d(TAG, "开始准备连接：" + bluetoothDevice.getName() + "-->" + bluetoothDevice.getAddress());
    ...
    private Runnable connectOutTimeRunnable = new Runnable() {
        ...
        public void run() {
            if(mBluetoothGatt == null){
                LogUtil.e(TAG,"connectOuttimeRunnable-->mBluetoothGatt == null");
                return;
            }
            if (ActivityCompat.checkSelfPermission(mContext, Manifest.permission.BLUETOOTH_CONNECT)
                    != PackageManager.PERMISSION_GRANTED) {
    ...
    private boolean setupService(BluetoothGatt bluetoothGatt,String serviceUUID,String readUUID,String writeUUID) {
        if (bluetoothGatt == null) {
            LogUtil.e(TAG, "setupService()-->bluetoothGatt == null");
            return false;
        }

        if(serviceUUID == null){
            LogUtil.e(TAG, "setupService()-->serviceUUID == null");
            return false;
        }

        for (BluetoothGattService service : bluetoothGatt.getServices()) {
//            LogUtil.d(TAG, "service = " + service.getUuid().toString());
...
//        bluetoothGattService = bleManager.getBluetoothGattService(bluetoothGatt,ConsData.MY_BLUETOOTH4_UUID);
        if (bluetoothGattService == null) {
            //找不到该服务就立即断开连接
            LogUtil.e(TAG, "setupService()-->bluetoothGattService == null");
            return false;
        }
        LogUtil.d(TAG, "setupService()-->bluetoothGattService = " + bluetoothGattService.toString());

        if(readUUID == null || writeUUID == null){
            LogUtil.e(TAG, "setupService()-->readUUID == null || writeUUID == null");
            return false;
        }
        ...
        if (readCharacteristic == null) {
            LogUtil.e(TAG, "setupService()-->readCharacteristic == null");
            return false;
        }
        if (writeCharacteristic == null) {
            LogUtil.e(TAG, "setupService()-->writeCharacteristic == null");
            return false;
        }
        //打开读通知
    ...
    public void enableNotification(boolean enable, BluetoothGatt gatt, BluetoothGattCharacteristic characteristic){
        if(gatt == null){
            LogUtil.e(TAG,"enableNotification-->gatt == null");
            return;
        }
        if(characteristic == null){
            LogUtil.e(TAG,"enableNotification-->characteristic == null");
            return;
        }
        //这一步必须要有，否则接收不到通知
    ...
    public boolean sendMessage(String msg){
        if(writeCharacteristic == null){
            LogUtil.e(TAG,"sendMessage(byte[])-->writeGattCharacteristic == null");
            return false;
        }

        if(mBluetoothGatt == null){
            LogUtil.e(TAG,"sendMessage(byte[])-->mBluetoothGatt == null");
            return false;
        }
        if (ActivityCompat.checkSelfPermission(mContext, Manifest.permission.BLUETOOTH_CONNECT)
                != PackageManager.PERMISSION_GRANTED) {
    ...
    public void disConnectDevice(){
        if(mBluetoothGatt == null){
            LogUtil.e(TAG,"disConnectDevice-->bluetoothGatt == null");
            return;
        }
...
//        mBluetoothGatt.close();
    ...
    public void openBluetooth(Context context,boolean isFast){
        ...
        if(!isEnable()){
            if(isFast){
                LogUtil.d(TAG,"直接打开手机蓝牙");
                bluetooth4Adapter.enable();  //BLUETOOTH_ADMIN权限
            }else{
                LogUtil.d(TAG,"提示用户去打开手机蓝牙");
                Intent enableBtIntent = new Intent(BluetoothAdapter.ACTION_REQUEST_ENABLE);
                context.startActivity(enableBtIntent);
            }
        }else{
            LogUtil.d(TAG,"手机蓝牙状态已开");
        }
    }
